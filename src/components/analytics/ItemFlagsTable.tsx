          ...
          <Table>
            ...
            <TableBody>
              {items.length === 0 ? (
                ...
                items.map((item) => {
                  const isExpanded = selectedItem === item.question_id;
                  const itemDetails = flagDetails.filter(detail => detail.question_id === item.question_id);
                  ...
                  return (
                    <>
                      <TableRow 
                        ...
                        onClick={() => setSelectedItem(isExpanded ? null : item.question_id)}
                      >
                        <TableCell>
                          <ChevronRight 
                          ...
                          <TableCell colSpan={6} className="bg-muted/30 p-0">
                            <div className="p-4 space-y-3">
                              <div className="flex items-center gap-2">
                                ...
                                <h4 className="font-semibold text-sm">User Feedback</h4>
                                <Badge variant="outline" className="ml-auto text-xs">
                                  {itemDetails.length} {itemDetails.length === 1 ? 'note' : 'notes'}
                                </Badge>
                              </div>
