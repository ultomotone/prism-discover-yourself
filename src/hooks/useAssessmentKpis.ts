    ...
    queryFn: async () => {
      ...
      return {
        ...
        summary: {
          ...
          totalCompleted,
          completionRate,
          avgTopGap: calibration?.avg_top_gap || 0,
          avgConfidence: calibration?.avg_confidence || 0,
          avgNPS: 0,
          avgClarity: 0,
          avgDropOffRate,
          avgEngagementRating: 0,
          medianCompletionTime,
          constructCoverageIndex: Array.isArray(data.coverage) && data.coverage.length > 0 
            ? data.coverage.reduce((sum: number, scale: any) => sum + (scale.coverage_pct || 0), 0) / data.coverage.length 
            : 0,
          difFlagRate: (fairness?.flagged_items || 0) / (fairness?.total_items || 1) * 100,
          calibrationError: calibration?.ece || 0,
          classificationStabilityRate: (classificationStability?.stability_rate || 0) * 100,
          freeToPaidRate: 0,
        },
      };
    },
