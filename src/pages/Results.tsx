...
export default function Results() {
      ...
      try {
        ...
        if (shareToken) {
          ...
          if (rpcErr) {
            ...
            console.debug('RPC get_profile_by_session failed', rpcErr.message);
          }
        }
        ...
        if (!isCompleted) {
          try {
            ...
            console.debug('Edge function fallback failed', e);
          }
        ...
        if (profileErr) {
          ...
          console.debug('Profile lookup failed', profileErr.message);
        }
        ...
        console.debug('Error fetching results:', err);
        if (!cancelled) {
          setError('server_error');
          setLoading(false);
        }
  ...
  const downloadPDF = async () => {
    try {
      ...
      if (!node) {
        console.debug('Results content not found');
        return;
      }
