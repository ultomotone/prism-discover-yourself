-- Fix failing view: replace invalid reference to r.user_id (assessment_responses has no user_id)
-- Create a robust v_sessions built from assessment_sessions + responses, with optional user_id from profiles
...
select
  ...
  s.started_at,
  s.completed_at,
  extract(epoch from (s.completed_at - s.started_at))::integer as duration_sec,
  r.response_count,
  r.first_answer_at,
  r.last_answer_at
