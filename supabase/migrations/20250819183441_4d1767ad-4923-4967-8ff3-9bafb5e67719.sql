-- ===== 0) SESSION START/END + DURATION + COMPLETION FLAG (fixed) =====
...
SELECT
  COALESCE(s.user_id, p.user_id) AS user_id,  -- prefer assessment_sessions; fallback profiles
  b.session_id,
  b.started_at,
  b.ended_at,
  CASE
    WHEN b.started_at IS NOT NULL AND b.ended_at IS NOT NULL
...
-- ===== 1) Foundation: (re)create v_retest_pairs (resilient) =====
    ...
    ddl := $SQL$
      ...
      WITH prof AS (
        SELECT
          ...
          p.session_id,
          p.created_at,
          -- pick the best available "strengths" source
          COALESCE(
...
-- ===== 2) Evidence KPI Views (safe order) =====

-- 2.1 Test–Retest: strength correlation
...
-- 2.3 State index (defensive if base tables missing)
DO $$
BEGIN
...
-- 2.5 Method agreement prep (FC vs Likert) (defensive)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='assessment_responses')
     AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='assessment_scoring_key') THEN
...
-- 2.7 Dimensionality reliability prep
...
-- 2.10 Fairness by demographics (country)
