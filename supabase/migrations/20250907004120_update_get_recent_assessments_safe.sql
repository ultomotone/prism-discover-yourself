...
BEGIN
  ...
  SELECT
    ...
    COALESCE(p.score_fit_calibrated, p.score_fit_raw,
      CASE
        WHEN p.top_types IS NOT NULL AND jsonb_array_length(p.top_types) > 0 AND p.type_scores IS NOT NULL
    ...
    CASE
      WHEN p.top_types IS NOT NULL AND jsonb_array_length(p.top_types) > 0 AND p.type_scores IS NOT NULL
      THEN COALESCE((p.type_scores->(p.top_types->>0)->>'share_pct')::numeric, 0)
      ELSE 0
    END as share_pct,
    p.fit_band,
    p.confidence,
