-- PRISM scoring & dashboard unification - replace existing views to use absolute fit and correct FC coverage

-- 1) Forced-Choice coverage for completed sessions only (keep same columns)
...
-- 2) FC analytics aggregated (keep same name/cols)
CREATE OR REPLACE VIEW public.v_fc_analytics AS
SELECT 
  fc_coverage_bucket,
...
-- 3) KPI metrics view - use absolute fit
...
-- 4) Latest assessments - use absolute fit consistently
...
-- 5) Fit histogram using absolute fit (keep columns)
...
-- 6) KPI overview 30d - use absolute fit and correct syntax
