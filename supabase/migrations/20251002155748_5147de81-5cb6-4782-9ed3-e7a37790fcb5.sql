-- Update get_assessment_kpis function to include retest data
CREATE OR REPLACE FUNCTION public.get_assessment_kpis(start_date date DEFAULT ((CURRENT_DATE - '7 days'::interval))::date, end_date date DEFAULT CURRENT_DATE)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'retest', (
      ...
      FROM (SELECT * FROM mv_kpi_retest) rt
    ),
    'cfa', (
