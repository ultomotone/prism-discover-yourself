-- ============================================================================
-- Schema Extensions & Comprehensive KPI System
-- ============================================================================

-- Add skip tracking and behavioral change columns
...
-- Create psychometric structure tables
...
-- Enable RLS
ALTER TABLE scale_catalog ENABLE ROW LEVEL SECURITY;
ALTER TABLE item_catalog ENABLE ROW LEVEL SECURITY;
ALTER TABLE psychometrics_external ENABLE ROW LEVEL SECURITY;
ALTER TABLE retest_invites ENABLE ROW LEVEL SECURITY;
...
-- ============================================================================
-- COMPREHENSIVE KPI MATERIALIZED VIEWS
-- ============================================================================
...
LEFT JOIN presented p ON p.question_id = q.id
LEFT JOIN answered a ON a.question_id = q.id
GROUP BY 1;
...
WITH daily_responses AS (
  SELECT date_trunc('day', ss.started_at)::date AS day, COUNT(*) AS response_count
...
daily_flags AS (
  SELECT date_trunc('day', fif.created_at)::date AS day, COUNT(*) AS flag_count
...
WHERE f.submitted_at >= CURRENT_DATE - INTERVAL '90 days'
GROUP BY 1, df.flag_count, dr.response_count;
CREATE UNIQUE INDEX IF NOT EXISTS mv_kpi_response_process_day_idx ON mv_kpi_response_process(day);

-- Reliability (requires item_catalog to be populated)
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_kpi_reliability_alpha AS
WITH item_scores AS (
  SELECT r.session_id, ic.scale_id, r.question_id, (COALESCE(r.answer_numeric,0) * COALESCE(ic.keyed,1))::numeric AS v
...
SELECT sv.scale_id,
  CASE WHEN sv.k > 1 AND sv.var_total > 0
    ...
    ELSE NULL END AS cronbach_alpha, sv.k
FROM scale_vars sv;
...
WITH numbered AS (
  SELECT ic.scale_id, r.session_id, r.question_id,
...
halves AS (
  SELECT scale_id, session_id,
    SUM(v) FILTER (WHERE rn % 2 = 1) AS odd_sum,
    SUM(v) FILTER (WHERE rn % 2 = 0) AS even_sum
...
LEFT JOIN psychometrics_external pe USING (scale_id)
GROUP BY 1,2,3;
...
SELECT scale_id, question_id, factor_loading, cfi, tli, rmsea, srmr
...
SELECT (COUNT(*) FILTER (WHERE dif_flag IS TRUE))::decimal / NULLIF(COUNT(*),0) AS dif_flag_rate
FROM psychometrics_external;

CREATE MATERIALIZED VIEW IF NOT EXISTS mv_kpi_calibration AS
WITH preds AS (
  SELECT conf_calibrated AS conf,
    CASE WHEN validity_status = 'pass' THEN 1 ELSE 0 END AS correct
  FROM profiles WHERE conf_calibrated IS NOT NULL
),
binned AS (
  SELECT WIDTH_BUCKET(conf, 0, 1, 10) AS bin, AVG(conf) AS p_expected,
    AVG(correct::int) AS p_observed, COUNT(*) AS n
  FROM preds GROUP BY 1
),
ece AS (SELECT SUM(ABS(p_expected - p_observed) * (n::decimal / (SELECT SUM(n) FROM binned))) AS ece FROM binned)
SELECT ece FROM ece;

-- Outcome Strength (fixed to use assessment_sessions.user_id)
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_kpi_classification_stability AS
WITH per_user AS (
  SELECT s.user_id, p.created_at AS submitted_at, p.type_code,
...
pairs AS (
  SELECT a.user_id, a.type_code AS type1, b.type_code AS type2,
    (b.submitted_at - a.submitted_at) AS delta
  FROM per_user a
  JOIN per_user b ON a.user_id=b.user_id AND b.rn=a.rn+1
...
SELECT VAR_POP(conf_calibrated) AS confidence_variance
FROM profiles WHERE conf_calibrated IS NOT NULL;

-- User Experience
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_kpi_user_experience AS
SELECT date_trunc('day', f.submitted_at)::date AS day,
  AVG(f.focus_ease) AS engagement_rating,
  AVG((f.actionability::int)) AS actionable_insights_pct,
  AVG(f.perceived_accuracy) AS accuracy_perception
FROM assessment_feedback f
WHERE f.submitted_at >= CURRENT_DATE - INTERVAL '90 days'
GROUP BY 1;
...
WITH completed AS (
  SELECT id AS session_id, user_id FROM assessment_sessions
...
joined AS (
  SELECT c.user_id, c.session_id, (p.session_id IS NOT NULL) AS saw_paywall, (pu.user_id IS NOT NULL) AS paid
  FROM completed c
  LEFT JOIN paywall p ON p.session_id = c.session_id
  LEFT JOIN purchases pu ON pu.user_id = c.user_id
)
SELECT AVG(paid::int)::decimal AS free_to_paid_rate, 0::decimal AS arpu_completed,
  (COUNT(*) FILTER (WHERE saw_paywall AND NOT paid))::decimal / NULLIF((COUNT(*) FILTER (WHERE saw_paywall)),0) AS paywall_abandon_rate
FROM joined;
...
SELECT AVG((behavioral_change::int))::decimal AS behavioral_impact_pct
FROM assessment_feedback WHERE behavioral_change IS NOT NULL;
...
joiner AS (SELECT b.conf, (f.behavioral_change::int) AS changed FROM base b 
  JOIN assessment_feedback f ON f.session_id = b.session_id
  WHERE b.conf IS NOT NULL AND f.behavioral_change IS NOT NULL)
SELECT CORR(conf, changed) AS trajectory_alignment_r FROM joiner;

-- Update cron schedules
SELECT cron.unschedule('mv_kpi_fast_10min');
SELECT cron.unschedule('mv_kpi_timing_30min');
SELECT cron.unschedule('mv_kpi_discrimination_hourly');
