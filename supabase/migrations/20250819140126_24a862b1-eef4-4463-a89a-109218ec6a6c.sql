...
CREATE OR REPLACE VIEW v_sessions_plus AS
SELECT
  v.*,
  s.started_at,
  s.completed_at,
...
CREATE OR REPLACE VIEW v_fit_ranks AS
WITH pairs AS (
  SELECT
    session_id,
...
CREATE OR REPLACE VIEW v_overlay_conf AS
...
CREATE OR REPLACE VIEW v_validity AS
SELECT
  session_id,
...
CREATE OR REPLACE VIEW v_fc_coverage AS
SELECT
  r.session_id,
...
CREATE OR REPLACE VIEW v_dim_coverage AS
SELECT
  r.session_id,
...
-- 9) Share entropy
CREATE OR REPLACE VIEW v_share_entropy AS
SELECT
  p.session_id,
  CASE 
...
CREATE OR REPLACE VIEW v_section_times AS
SELECT
  r.session_id, 
...
CREATE OR REPLACE VIEW v_conf_dist AS
