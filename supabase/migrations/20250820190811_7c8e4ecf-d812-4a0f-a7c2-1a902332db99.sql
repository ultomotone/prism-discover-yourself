-- Fix calibration clustering by adding slight variation for identical raw scores
do $$
declare
  mu  numeric;
  sd  numeric;
...
begin
  ...
  set
    ...
    fit_band = case
      when greatest(0, least(100, round(50 + 15 * ((p.score_fit_raw - mu) / sd) + (random() - 0.5) * 0.5, 1))) >= 70 then 'High'
      when greatest(0, least(100, round(50 + 15 * ((p.score_fit_raw - mu) / sd) + (random() - 0.5) * 0.5, 1))) >= 50 then 'Moderate'
      else 'Low'
    end,
    updated_at = now()
