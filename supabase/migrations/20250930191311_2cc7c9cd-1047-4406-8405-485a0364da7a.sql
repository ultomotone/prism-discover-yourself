-- Drop and recreate get_assessment_kpis to include all new metrics
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'business', (
      SELECT COALESCE(row_to_json(b), '{}'::jsonb)
      FROM (SELECT * FROM mv_kpi_business LIMIT 1) b
    ),
    'followup', (
      SELECT COALESCE(row_to_json(fu), '{}'::jsonb)
      FROM (SELECT * FROM mv_kpi_followup LIMIT 1) fu
    ),
    'behavioralImpact', (
      SELECT COALESCE(row_to_json(bi), '{}'::jsonb)
      FROM (SELECT * FROM mv_kpi_behavioral_impact LIMIT 1) bi
    ),
    'trajectoryAlignment', (
      SELECT COALESCE(row_to_json(ta), '{}'::jsonb)
