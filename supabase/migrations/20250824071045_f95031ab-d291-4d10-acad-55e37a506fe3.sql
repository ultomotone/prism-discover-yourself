-- Fix PRISM scoring & dashboard unification issues
...
-- 7) Update KPI overview to use absolute fit
...
SELECT 
  ...
  -- Quality metrics using absolute fit
  ROUND(COALESCE(AVG(fit), 0), 1) as avg_fit_score,
  ROUND(
    COUNT(*)::numeric FILTER (WHERE confidence IN ('High', 'Moderate')) * 100.0 / 
    NULLIF(COUNT(*), 0),
    2
  ) as hi_mod_conf_pct
