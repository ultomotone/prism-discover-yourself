-- Fix RPC column ambiguity by qualifying parameter references
...
-- Optional performance index (idempotent)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE schemaname='public' AND tablename='assessment_sessions'
      AND indexname='idx_assessment_sessions_id_share_token'
  ) THEN
    CREATE INDEX idx_assessment_sessions_id_share_token
