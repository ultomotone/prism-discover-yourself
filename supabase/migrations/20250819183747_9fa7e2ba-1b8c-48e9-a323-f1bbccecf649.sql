-- Create corrected views with proper scale types
...
SELECT 
    ...
    -- Get top types from the profiles table
    (SELECT p1.top_types->>0 FROM profiles p1 WHERE p1.session_id = rp.session_id_1) AS top1_a,
    (SELECT p2.top_types->>0 FROM profiles p2 WHERE p2.session_id = rp.session_id_2) AS top1_b,
    ((SELECT p1.top_types->>0 FROM profiles p1 WHERE p1.session_id = rp.session_id_1) = 
     (SELECT p2.top_types->>0 FROM profiles p2 WHERE p2.session_id = rp.session_id_2)) AS stable,
    ((SELECT p1.top_types->>0 FROM profiles p1 WHERE p1.session_id = rp.session_id_1) <> 
     (SELECT p2.top_types->>0 FROM profiles p2 WHERE p2.session_id = rp.session_id_2)
     AND (SELECT p1.top_types->>1 FROM profiles p1 WHERE p1.session_id = rp.session_id_1) = 
         (SELECT p2.top_types->>0 FROM profiles p2 WHERE p2.session_id = rp.session_id_2)
     AND (SELECT p1.top_types->>0 FROM profiles p1 WHERE p1.session_id = rp.session_id_1) = 
         (SELECT p2.top_types->>1 FROM profiles p2 WHERE p2.session_id = rp.session_id_2)) AS adjacent_flip
...
WHERE ask.fc_map IS NOT NULL 
  ...
  AND (ask.scale_type = 'LIKERT_1_7' OR ask.scale_type LIKE 'FORCED_CHOICE_%');
