...
-- 5) Integrity check function (fixed syntax)
...
BEGIN
  ...
  IF required_tags IS NOT NULL AND jsonb_typeof(required_tags) = 'array' THEN
    FOR tag_rec IN 
      SELECT jsonb_array_elements_text(required_tags) AS tag_name
    LOOP
      IF NOT EXISTS (
        SELECT 1 FROM public.assessment_questions WHERE tag = tag_rec.tag_name
      ) THEN
        missing_tags := array_append(missing_tags, tag_rec.tag_name);
      END IF;
    END LOOP;
