-- Verify refresh_all_materialized_views includes STATE overlay MVs
-- This updates the function to ensure STATE MVs are refreshed
...
BEGIN
  -- STATE Overlay MVs (must be first due to dependencies)
  BEGIN REFRESH MATERIALIZED VIEW CONCURRENTLY mv_state_overlay_daily;
  EXCEPTION WHEN OTHERS THEN v_errors := v_errors || jsonb_build_object('view', 'mv_state_overlay_daily', 'error', SQLERRM); END;
  
  BEGIN REFRESH MATERIALIZED VIEW CONCURRENTLY mv_kpi_state_overlay;
  EXCEPTION WHEN OTHERS THEN v_errors := v_errors || jsonb_build_object('view', 'mv_kpi_state_overlay', 'error', SQLERRM); END;
  ...
  -- Psychometric views (with real session data)
