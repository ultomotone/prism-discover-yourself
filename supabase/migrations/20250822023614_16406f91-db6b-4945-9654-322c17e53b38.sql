-- Complete dashboard statistics rewrite with backfill
...
-- Keep existing zero-arg function for UI compatibility (updates today)
...
-- Trigger to update stats when a profile is created (uses the session's completed_at date)
...
DECLARE
  start_date date;
BEGIN
  SELECT COALESCE(MIN(DATE(completed_at)), CURRENT_DATE) INTO start_date 
  FROM public.assessment_sessions 
  WHERE status = 'completed';
  
  PERFORM public.update_dashboard_statistics_range(start_date, CURRENT_DATE);
END;
$$;