-- Fix v1.1 backfill: compute calibrated fit from type_scores and create activity view (corrected)
...
-- Step 2: Create activity map view for country data using assessment_responses
create or replace view v_activity_country_30d as
select
  coalesce(
    upper(nullif(trim(ar.answer_value), '')),
    'XX'
  ) as iso2,
  coalesce(ar.answer_value, 'Unknown') as country_label,
  count(distinct p.session_id) as sessions
from profiles p
left join assessment_responses ar on ar.session_id = p.session_id 
  and ar.question_text ilike '%country%'
where p.created_at >= now() - interval '30 days'
  and ar.answer_value is not null
  and trim(ar.answer_value) != ''
group by ar.answer_value
having count(distinct p.session_id) > 0
order by sessions desc;

-- Step 3: Grant proper RLS permissions for service role updates
drop policy if exists "Service role can update profiles for backfill" on profiles;
