...
test('filters normalize all to null', async () => {
  (globalThis as any).window = {
    __APP_CONFIG__: {
      SUPABASE_URL: 'https://example.supabase.co',
      SUPABASE_ANON_KEY: 'key',
  ...
  const supabaseModule = await import('../src/lib/supabase/client');
  let params: any;
  (supabaseModule.supabase as any).rpc = async (_fn: string, p: any) => {
    params = p;
    return { data: sampleSummary, error: null };
  };
  const { fetchSummaryKPIs } = await import('../src/lib/api/admin');
  await fetchSummaryKPIs({
    ...filters,
    overlay: 'all',
    confidence: 'all',
    type: 'all',
  ...
  assert.equal(params.overlay, null);
  assert.equal(params.confidence, null);
  assert.equal(params.type, null);
  assert.equal(params.device, null);
  delete (globalThis as any).window;
});
